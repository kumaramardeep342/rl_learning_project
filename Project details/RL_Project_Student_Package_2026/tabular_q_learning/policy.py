...
MODEL_PATH = Path(__file__).parent / "q_table_tabular.pkl"
...
def run_policy(observation):
    """Deterministic inference for Tabular Q-Learning."""
    state = _get_state(observation)

    if state in Q_TABLE:
        action_idx = int(np.argmax(Q_TABLE[state]))
    else:
        # Fallback to conservative order if state was unseen during training
        action_idx = 0
